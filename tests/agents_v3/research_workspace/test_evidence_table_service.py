...
class TestEvidenceTableService:
    ...
    def test_query_by_scope_topic(self, service, sample_card):
        service.build_for_project("proj1")
        scope = RetrievalScope(
    ...
    def test_build_no_cards(self, service):
        records = service.build_for_project("empty_proj")
        assert len(records) == 0


class TestNormalizeTopics:
    def test_normalize_similar_topics(self):
        cards = [
    ...
    def test_normalize_distinct_topics(self):
        cards = [
