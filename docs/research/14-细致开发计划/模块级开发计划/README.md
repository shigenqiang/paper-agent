...
本目录下 `00-15` 每个模块文件都已补充同名章节，用于把真实代码状态、已知缺陷、下游集成映射、API 定义、增强测试计划和优先级排序的下一步收束到模块级开发计划中。

当前深化分两轮：

**第一轮（2026-05-29）**：全部 `00-15` 补充”当前代码对齐深化”章节。

**第二轮（2026-06-01）**：对核心模块做深度代码对齐，补充实现状态、已知缺陷、下游集成、API 定义、增强测试计划和优先级下一步。已完成深度对齐的模块：

| 模块 | 深化内容 | 代码规模 | 已知缺陷 | 测试覆盖 |
| --- | --- | --- | --- | --- |
| `05-论文卡片生成模块` | 实现状态 + 7 缺陷 + 下游映射 + 5 API + 测试 21→~50 | 637 行 / 22 方法 | PromptRegistry 未接入、invoke_structured 未使用 | 21 已有 |
| `08-RetrievalScope模块` | R1.4 纠正 + 8 缺陷 + 下游消费 + 7 API + 测试 22→~47 | ~580 行 / 9+12 方法 | Gap 节点解析不完整、context_budget 未实现 | 22 已有 |
| `10-综述生成模块` | 10 缺陷 + Review-Revise 设计 + context compression + 测试 12→~37 | 569 行 / 12 方法 | graph_context 未使用、ReportService 未接入 | 12 已有 |
| `12-报告版本与导出模块` | 实现状态 + 10 缺陷 + 下游映射 + 8 API + 测试 22→~79 | 388 行 / 14 方法 | PG schema 不完整、export 写副作用、生成模块绕过 | 22 已有 |
...
7. 生成模块接入 ReportService：review_generator 和 innovation_generator 改用 save_report，自动创建版本和来源索引。
8. 结构化输出迁移：所有 generator 从 invoke_json 迁移到 invoke_structured，PromptRegistry 统一管理 prompt 版本。
```
...
截至 2026-06-01，本目录中 `00` 到 `15` 均已有专项开发计划。其中 4 个核心模块已完成第二轮深度代码对齐（含实现状态、已知缺陷、下游集成、API 定义、增强测试计划和优先级下一步）。
...
| `01-核心模型与存储模块.md` | 已完成 |
| `02-项目服务模块.md` | 已完成 |
| `03-论文库搜索导入模块.md` | 已完成，已按当前搜索实现和平台选型深化 |
| `04-PDF解析与分块模块.md` | 已完成 |
| `05-论文卡片生成模块.md` | ✅ 深度对齐完成（637 行 / 7 缺陷 / 测试 21→~50） |
| `06-证据表模块.md` | 已完成 |
| `07-知识图谱模块.md` | 已完成 |
| `08-RetrievalScope模块.md` | ✅ 深度对齐完成（~580 行 / 8 缺陷 / 测试 22→~47） |
| `09-ScopeQA与RAG模块.md` | 已完成 |
| `10-综述生成模块.md` | ✅ 深度对齐完成（569 行 / 10 缺陷 / 测试 12→~37） |
| `11-创新点报告模块.md` | 已完成 |
| `12-报告版本与导出模块.md` | ✅ 深度对齐完成（388 行 / 10 缺陷 / 测试 22→~79） |
