...
|------|---------|-----------|---------|------|
| 场景1 | 开 | 关 | 关 | 仅排序（基线） |
| 场景2 | 开 | 0.85 | 关 | 排序 + 低分过滤（零额外开销） |
| 场景3 | 开 | 关 | 开 | 排序 + LLM相关性判断（严格） |
| 场景4 | 开 | 0.85 | 开 | 全部开启（推荐） |

## 结果汇总

| 方案 | 耗时 | 总数 | 相关 | 不相关 | 精准率 |
|------|------|------|------|--------|--------|
| 场景1: 仅排序 | 9.79s | 19 | 11 | 8 | 58% |
| 场景2: 排序 + min_score≥0.85 | 9.79s | 16 | 11 | 5 | 69% |
| 场景3: 排序 + LLM过滤 | 78.51s | 2 | 2 | 0 | 100% |
| 场景4: 排序 + min_score + LLM（推荐） | 48.99s | 3 | 3 | 0 | 100% |
...
- 耗时: 9.79s（与场景1相同，零额外开销）
- 精准率: 69% (11/16)
- 过滤效果: 19篇 → 16篇，去掉了3篇低分论文

**被过滤掉的论文（分数 < 0.85）：**

| # | 分数 | 标题 | 相关 | 过滤原因 |
|---|------|------|------|---------|
| 17 | 0.848 | Differentiable biology: using deep learning for biophysics | N | 低分 + 不相关 |
| 18 | 0.847 | The topology of fMRI-based networks defines the performance | N | 低分 + 不相关 |
| 19 | 0.841 | Deep neural network with weight sparsity control and pre-training | N | 低分 + 不相关 |

**分析：** min_score=0.85 成功过滤掉了3篇不相关论文（精准率 58% → 69%），但排名靠前的4篇不相关论文（Continual Learning、XGBoost、NumPy 等）因为分数高于0.85未被过滤。这说明 min_score 只能过滤"既不相关又低分"的论文，无法过滤"不相关但高分"的论文。
