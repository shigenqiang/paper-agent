...
│                                                                              │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │ Phase 1: DIAGNOSTIC — 问题诊断 (并行)                                  │  │
│  │                                                                        │  │
│  │   TopicRefiner + LiteratureMapper + MethodologyAdvisor (并行)         │  │
│  │        │                      │                      │                  │  │
│  │        └──────────────────────┼──────────────────────┘                  │  │
│  │                               ▼                                           │  │
│  │                    DiagnosticResult → Problems Found                   │  │
│  │                               │                                           │  │
│  │                               ▼                                           │  │
│  │                    severity ≥ 0.6 → _apply_fixes() 针对性修复           │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
...
│  │        │                                                               │  │
│  │        ├── _analyze_domain() → 领域分析                                │  │
│  │        ├── _generate_topic_candidates() → 生成5个候选主题              │  │
│  │        ├── _evaluate_feasibility() → 评估可行性                        │  │
│  │        │        (literature_adequacy, method_feasibility, novelty)     │  │
│  │        └── _select_best_topic() → 选择最佳主题                          │  │
│  │        │                                                               │  │
│  │        ▼                                                               │  │
│  │   {selected_topic, alternative_topics, domain_analysis}               │  │
│  │        │                                                               │  │
│  │        ▼                                                               │  │
...
│  │                                                                        │  │
│  │   LiteratureAgent.execute({topic, research_question})                   │  │
│  │        │                                                               │  │
│  │        ├── _generate_search_queries() → 生成8-12个搜索查询              │  │
│  │        │        (多角度: 基础/扩展/验证)                                │  │
│  │        │                                                              │  │
│  │        ├── _multi_engine_search() → 多引擎并行搜索                      │  │
│  │        │        │                                                      │  │
│  │        │        ├── PaperSearchAgent.execute()                         │  │
│  │        │        │   (ArXiv / PubMed / Semantic Scholar / OpenAlex)     │  │
│  │        │        └── asyncio.gather() 并发执行                          │  │
│  │        │                                                              │  │
│  │        ├── _compute_paper_relevance() → 嵌入向量计算相关性               │  │
│  │        │        (ModelScope Qwen3-Embedding-0.6B, 余弦相似度)            │  │
│  │        │                                                              │  │
│  │        ├── _deep_read() → 过滤 embedding_relevance > 0.85             │  │
│  │        │        (最多20篇深度阅读)                                       │  │
│  │        │                                                              │  │
│  │        └── _identify_gaps() → 识别3-5个研究空白                        │  │
│  │                 │                                                      │  │
│  │                 ▼                                                      │  │
│  │   {papers, paper_analyses, research_gaps, search_queries}           │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
...
│  │                                                                        │  │
│  │   MethodologyAdvisor.analyze(topic, literature_result)                │  │
│  │        │                                                               │  │
│  │        ▼                                                               │  │
...
│  │                                                                        │  │
│  │   5.1 OutlineAgent.execute({thesis, literature})                     │  │
│  │   │        │                                                           │  │
│  │   │        ├── _design_structure() → 设计论文结构                       │  │
│  │   │        │        (EnglishFirstMixin: 英文优先处理)                   │  │
│  │   │        │        → PaperStructure {title, chapters, paper_type}    │  │
│  │   │        │                                                           │  │
│  │   │        ├── _plan_chapters() → 规划章节内容                          │  │
│  │   │        │        → main_points, citations_needed, content_guidance │  │
│  │   │        │                                                           │  │
│  │   │        └── _identify_key_arguments() → 确定关键论点                │  │
│  │   │                 │                                                   │  │
│  │   │                 ▼                                                   │  │
│  │   │   {outline: {structure, chapters, key_arguments}}                │  │
│  │   │        │                                                           │  │
│  │   │        ▼                                                           │  │
│  │   │   HITLManager.interrupt("outline_review") — 用户确认大纲           │  │
│  │   │                                                           │        │  │
│  │   │   ┌─────────────────────────────────────────────────────────────┐   │  │
│  │   │   │ DraftWriterAgent.execute({outline}, context)                │   │  │
│  │   │   │        │                                                   │   │  │
│  │   │   │        ├── 独立章节并行撰写 (asyncio.gather)                │   │  │
│  │   │   │        │   _write_chapter()                                 │   │  │
│  │   │   │        │   ├── _write_introduction()                        │   │  │
...
│  │   │   │        │   ├── _write_discussion()                           │   │  │
│  │   │   │        │   └── _write_conclusion()                          │   │  │
│  │   │   │        └── _compile_draft() → 合并完整初稿                   │   │  │
│  │   │   └─────────────────────────────────────────────────────────────┘   │  │
│  │   │        │                                                           │  │
│  │   │        ▼                                                           │  │
│  │   │   ReviewerAgent.review({draft}) → {quality_score, issues}         │  │
│  │   │        │                                                           │  │
│  │   │        ▼                                                           │  │
│  │   │   Evaluator.evaluate(draft) → avg_score (7维度)                  │  │
│  │   │        │                                                           │  │
│  │   │        ├── score >= 0.7 → 通过                                    │  │
│  │   │        └── score < 0.7 → 迭代修复 (最多 max_iterations=3)        │  │
│  │   │                                                           │        │  │
│  │   └───────────────────────────────────────────────────────────────────┘   │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
...
│  │ Phase 6: POLISH — 润色阶段                                            │  │
│  │                                                                        │  │
│  │   LanguagePolisherAgent.execute({text, language})                     │  │
│  │   │     │                                                              │  │
│  │   │     ├── _parse_diagnosis_response() → 诊断JSON解析                │  │
│  │   │     │        (grammar_issues, style_issues, terminology_issues)   │  │
│  │   │     │                                                              │  │
│  │   │     ├── _clean_text_output() → 清理输出                           │  │
│  │   │     │        (移除思考块、代码块、诊断字段)                        │  │
│  │   │     │                                                              │  │
│  │   │     └── _fallback_polish() → 降级润色 (诊断失败时)                 │  │
│  │   │                                                                       │  │
│  │   ReportRefinerAgent.execute({draft, max_iterations})                 │  │
│  │   │     │                                                               │  │
│  │   │     ├── _review_draft() → 评审 (6维度)                            │  │
│  │   │     │        (structure, logic, argumentation, citation, language)│  │
│  │   │     │                                                              │  │
│  │   │     ├── _refine_draft() → 针对性修复                              │  │
│  │   │     │                                                              │  │
│  │   │     ├── _final_polish() → 最终润色                                 │  │
...
│  │   │                                                                       │  │
│  │   SmartReviserAgent.execute({original_text, feedback})                │  │
│  │        │                                                               │  │
│  │        ├── _parse_feedback() → 解析意见                                │  │
│  │        ├── _categorize_feedback() → 分类 (content/structure/language) │  │
│  │        ├── _apply_revisions() → 执行修改                              │  │
│  │        └── _generate_revision_report() → 生成报告                      │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  最终输出:                                                                   │
│  {                                                                           │
...
│                           "methodology", "writing", "polish"],               │
│      "final_paper": "...",  # _clean_final_paper() 清理论文                  │
│      "quality_history": [{"score": 0.85, "level": "good"}, ...],          │
│      "final_quality": 0.85,                                                  │
│      "iterations": 2,                                                        │
...
| DIAGNOSTIC | 诊断 | TopicRefiner + LiteratureMapper + MethodologyAdvisor | problem_oriented | 问题诊断 |

### 5.3 MasterSupervisor 质量阈值
...
PaperDB.find_duplicates(paper: dict) -> List[dict]:
    ...
    # 2. 标题相似度匹配 (模糊) - 使用 embedding cosine_similarity
    if paper.get("embedding") and paper.get("title"):
        similar = self.cosine_similarity(
            paper["embedding"],
            threshold=0.95
        )
        if similar:
