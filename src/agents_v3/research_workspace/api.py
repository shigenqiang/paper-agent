...
from src.agents_v3.research_workspace.api_deps import (
    get_card_generator,
...
from src.agents_v3.research_workspace.api_errors import (
    APIError,
    NotFoundError,
    api_error_handler,
    generic_error_handler,
)
from src.agents_v3.research_workspace.api_models import (
    ApiMeta,
    ApiResponse,
...
def create_app(storage=None) -> FastAPI:
    ...
    def get_subgraph(project_id: str, node_ids: list[str], hops: int = 1):
        svc = get_graph_service()
        result = svc.get_subgraph(project_id, node_ids, hops=hops)
        return ApiResponse(data=result)

    @app.get("/api/rw/projects/{project_id}/kg/gaps")
    def find_gaps(project_id: str, min_confidence: float = 0.5):
        svc = get_graph_service()
        gaps = svc.find_gaps(project_id, min_confidence=min_confidence)
    ...
    def resolve_scope(project_id: str, req: ScopeResolveRequest):
        ...
        scope = svc.resolve(project_id, req.model_dump())
        return ApiResponse(data=scope.model_dump())

    @app.get("/api/rw/projects/{project_id}/scope/filters")
    def get_scope_filters(project_id: str):
        svc = get_scope_service()
        filters = svc.get_scope_filters(project_id)
    ...
    def list_reports(project_id: str, type: str | None = None):
        ...
        reports = svc.list_reports(project_id, type)
        return ApiResponse(data=[r.model_dump() for r in reports])
    ...
    def generate_review(project_id: str, req: ReportGenerateRequest):
        ...
        report = svc.generate(project_id, req.scope.model_dump(), req.options)
        return ApiResponse(data=report.model_dump())
    ...
    def list_tasks(project_id: str | None = None, status: str | None = None):
        svc = get_task_service()
        tasks = svc.list_tasks(project_id, status)
